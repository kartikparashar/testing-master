...
root.title('HISTORICAL DATA WITH MOVING AVERAGE ')
...
def getdata():
   ...
   # b = data["close"].rolling(int(box_meanval)).mean()

    b = pd.rolling_mean(dat["close"],int(entm1))

    #b = pd.rolling_mean(data["close"], int(box_meanval))
    # b = pd.rolling_mean(meanfinal["close"],int(entm1))

    rename = entm1 + 'day MovingAvg'
    b = b.rename(columns={'close': rename})
    b = pd.DataFrame(b)
    new = pd.concat([dat, b], axis=1)
    print(new)
    base_dir = pat3
    global fname
    global ffname
...
mean= Label(root,bg='black',fg='orange', text='GET MOVING AVERAGE',font='VERONICA')
mean.grid(row=22,column=0)


meanent= Label(root,bg='black',fg='white', text='ENTER MOVING AVERAGE')
meanent.grid(row=23,column=0)
entmean = Entry(root)
entmean.grid(row=23, column=1)

#getdatamean
newmean = Button(root, text="FETCH MOVING AVERAGE", fg="white", bg='red',command=getdata)
newmean.grid(row=24, column=1)
