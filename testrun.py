...
signalresult =pd.DataFrame({'price' : [],'avgprice' : [],'qty' : [],'signal' : [],'time' : []})
...
def fetchexe():
    ...
    signalresult=signalresult.append({'price' : exeprice,'avgprice' : avgprice,'qty' : exeqty,'signal' : exesignal,'time' : exetime},ignore_index=True)
...
def calqty(p_amt):
    cap_amt = 100000
    quant = max(int(round((cap_amt)/(p_amt * 50))),1)
    return quant

##entry function
def entryfun(signal,price):
    global sflag#,o_type,qty):
    sflag = 0
    cnt = -1
    for j in range(10):
...
for i in range(times):
    ...
    if (signal == 'n'):
            time.sleep(3596)  #need to confirm sleep time and loop
    elif (signal != 'n'):
        tws.reqIds(1)
        order_id = callback.next_ValidId + 1
        callback.next_ValidId = order_id
        ...
        if ((filled== qty) and (remain==0)):
            ...
            time.sleep(3589)
             ...
             if ((tf1 == 1) and (new_entry == 'T')) or ((tf2 == 1) and (new_entry == 'T')) :#exit criteria
                 ...
                 time.sleep(3589)  #need to confirm
    ...
    #partial fill
             ...
             if ((tf1 == 1) and (new_entry == 'T')) or ((tf2 == 1) and (new_entry == 'T')) :#exit
                 ...
                 # to be written in csv ,do not re write the existing price variable
                 stime = (3589 - (cnt * 30))
                 time.sleep(stime)  #need to confirm
